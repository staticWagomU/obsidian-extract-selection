...
const scrum: ScrumDashboard = {
  ...
  product_backlog: [
    {
      id: "PBI-004",
      story: { role: "ユーザー", capability: "日付フォーマットに基づいたファイル名を自動生成したい", benefit: "ファイル名を毎回考えなくて済むようにするため" },
      acceptance_criteria: [
        { criterion: "yyyyMMddHHmmss形式でファイル名が生成される", verification: "npm test -- --run -t 'filename.*yyyyMMddHHmmss'" },
  ...
  sprint: {
    ...
    subtasks: [
      ...
      },
      {
        ...
        status: "green",
        commits: [],
        notes: ["Parameterized Testパターンを使用して各プレースホルダーを個別にテスト", "実装はSubtask 1で完了済み、Subtask 2では包括的なテストケースを追加"],
      },
      {
